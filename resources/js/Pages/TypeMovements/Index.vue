<template>
    <a-row>
        ...
        <a-col :span="12">
            <div v-if="permissions.typeMovementsCreate">
                <Link href="/typeMovements/create" class="ant-btn ant-btn-primary ant-btn-round ant-btn-sm" as="button">Crear
                </Link>
            </div>
                        ...
                        <div class="ant-table-content">
                            <table style="table-layout: auto;" class="ant-table-striped">
                                ...
                                <tbody class="ant-table-tbody">
                                    <tr class="ant-table-row ant-table-row-level-0" v-for="typeMovement in typeMovements.data"
                                        :key="typeMovement.id">
                                        <td class="ant-table-cell" colstart="0" colend="0">{{ typeMovement.id }}</td>
                                        <td class="ant-table-cell" colstart="1" colend="1">{{ typeMovement.name }}</td>
                                        <td class="ant-table-cell" colstart="3" colend="4">
                                            <a-dropdown-button>
                                                Acciones
                                                <template #overlay>
                                                    <a-menu>
                                                        <a-menu-item key="1" v-if="permissions.typeMovementsEdit">
                                                            <form-outlined />
                                                            <Link :href="`/typeMovements/edit/${typeMovement.id}`"
                                                                class="ant-btn ant-btn-default ant-btn-round ant-btn-sm"
                                                                as="button">Editar</Link>
                                                        </a-menu-item>
                                                        <a-menu-item key="2" v-if="permissions.typeMovementsDestroy">
                                                            <delete-outlined />
                                                            <a-popconfirm title="Estas seguro de la operación?"
                                                                ok-text="Si" cancel-text="No"
                                                                @confirm="borrar(typeMovement.id)">
                                                                <button
                                                                    class="ant-btn ant-btn-default ant-btn-round ant-btn-sm">Borrar</button>
                                                            </a-popconfirm>
                                                        </a-menu-item>
                                                        <a-menu-item key="3" v-if="permissions.typeMovementsShow">
                                                            <eye-outlined />

                                                            <Link :href="`/typeMovements/show/${typeMovement.id}`"
                                                                class="ant-btn ant-btn-default ant-btn-round ant-btn-sm"
                                                                as="button">Ver</Link>
    ...
    <paginator :datos="typeMovements"> </paginator>
</template>
...
export default {
    ...
    props: ["typeMovements", "filters", "sysMessage", 'permissions'],
    ...
    setup(props) {
        ...
        const borrar = (id) => {
            ...
            Inertia.delete(`/typeMovements/delete/${id}`)
            //}
        };
