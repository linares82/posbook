<template>
    <a-row>
        <a-col :span="12">
            <a-page-header style="border: 1px solid rgb(235, 237, 240)" title="Productos" sub-title="Lista" />
        </a-col>
        <a-col :span="12">
            <div v-if="permissions.productsCreate">
                <Link href="/products/create" class="ant-btn ant-btn-primary ant-btn-round ant-btn-sm" as="button">Crear
                </Link>
            </div>
                        ...
                        <div class="ant-table-content">
                            <table style="table-layout: auto;" class="ant-table-striped">
                                ...
                                <tbody class="ant-table-tbody">
                                    <tr class="ant-table-row ant-table-row-level-0" v-for="product in products.data"
                                        :key="product.id">
                                        <td class="ant-table-cell" colstart="0" colend="0">{{ product.id }}</td>
                                        <td class="ant-table-cell" colstart="1" colend="1">{{ product.name }}</td>
                                        <td class="ant-table-cell" colstart="3" colend="4">
                                            <a-dropdown-button>
                                                Acciones
                                                <template #overlay>
                                                    <a-menu>
                                                        <a-menu-item key="1" v-if="permissions.productsEdit">
                                                            <form-outlined />
                                                            <Link :href="`/products/edit/${product.id}`"
                                                                class="ant-btn ant-btn-default ant-btn-round ant-btn-sm"
                                                                as="button">Editar</Link>
                                                        </a-menu-item>
                                                        <a-menu-item key="2" v-if="permissions.productsDestroy">
                                                            <delete-outlined />
                                                            <a-popconfirm title="Estas seguro de la operación?"
                                                                ok-text="Si" cancel-text="No"
                                                                @confirm="borrar(product.id)">
                                                                <button
                                                                    class="ant-btn ant-btn-default ant-btn-round ant-btn-sm">Borrar</button>
                                                            </a-popconfirm>
                                                        </a-menu-item>
                                                        <a-menu-item key="3" v-if="permissions.productsShow">
                                                            <eye-outlined />
    ...
    <paginator :datos="products"> </paginator>
</template>
...
export default {
    ...
    props: ["products", "filters", "sysMessage", 'permissions'],
    ...
    setup(props) {
        ...
        const borrar = (id) => {
            ...
            Inertia.delete(`/products/delete/${id}`)
            //}
        };
