<template>
  <a-row>
    <a-col :span="12">
      <a-page-header style="border: 1px solid rgb(235, 237, 240)" title="Corteos" sub-title="Editar" />
    </a-col>
    <a-col :span="12">
    ...
    <a-row>
      <a-col :md="7">
        <a-form-item label="Nombre" name="name">
          <a-input v-model:value="formCorte.name"> </a-input>
...
export default {
  ...
  setup(props) {
    let formCorte = reactive({
      id: props.corte.id,
      name: props.corte.name,
    });

    let processing = ref(false);

    let submitF = () => {
      Inertia.post("", formCorte, {
        onStart: () => {
